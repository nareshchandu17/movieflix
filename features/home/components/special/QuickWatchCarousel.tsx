    ...
    const fetchQuickWatch = async () => {
      try {
        ...
        const responses = await Promise.all([
          api.discover("movie", { "with_runtime.lte": 95, "vote_count.gte": 50, sortBy: "popularity.desc", page: 1 }),
          api.discover("movie", { "with_runtime.lte": 95, "vote_count.gte": 50, sortBy: "popularity.desc", page: 2 }),
        ]);

        const allMovies: TMDBMovie[] = [];
        responses.forEach((res: any) => allMovies.push(...(res.results || [])));

        const uniqueMovies = allMovies
          .filter((m, i, self) => i === self.findIndex((x) => x.id === m.id))
          .slice(0, 30);

        setQuickWatch(uniqueMovies);
      } catch (err) {
