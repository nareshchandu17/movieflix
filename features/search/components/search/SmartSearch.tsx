...
const SmartSearch = () => {
  ...
  const searchWithNLP = async (searchQuery: string): Promise<SearchResult[]> => {
    ...
    try {
      ...
      const movieResults: SearchResult[] = rawResults.filter((item: any) => (item.media_type === 'movie' || 'title' in item)).map((item: any) => ({
        ...
        releaseDate: item.release_date, popularity: item.popularity, raw: item
      }));

      const tvResults: SearchResult[] = rawResults.filter((item: any) => (item.media_type === 'tv' || 'name' in item) && !('title' in item)).map((item: any) => ({
        ...
        rating: item.vote_average || 0, description: item.overview || "", genres: [], trending: (item.popularity || 0) > 40,
        releaseDate: item.first_air_date, popularity: item.popularity, raw: item
      }));
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setQuery(value);
    setInputWidth(value.length > 0 ? "w-72 sm:w-80" : (isFocused ? "w-64 sm:w-72" : "w-44"));
