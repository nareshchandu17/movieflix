          ...
          {results.slice(0, 15).map((item, index) => (
            ...
            >
              <div 
                ...
                onClick={() => router.push(`/${mediaType === 'tv' ? 'series' : 'movie'}/${item.id}`)}
              >
                <Image
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 translate-y-2 group-hover/card:translate-y-0 transition-transform duration-300 flex flex-col justify-end min-h-[50%]">
                  ...
                  <div className="flex items-center gap-2 mt-3 opacity-0 group-hover/card:opacity-100 transition-all duration-300 translate-y-1 group-hover/card:translate-y-0">
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/${mediaType === 'tv' ? 'series' : 'movie'}/${item.id}`);
                      }}
                      className="flex-1 bg-white hover:bg-red-600 text-black hover:text-white py-1.5 rounded-lg text-[9px] font-black transition-all flex items-center justify-center gap-1 uppercase tracking-tighter"
                    >
                    ...
                    <button 
                      ...
                      className="w-7 h-7 rounded-lg bg-white/10 backdrop-blur-md border border-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
                    >
