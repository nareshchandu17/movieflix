...
export function VideoLayer({ url }: VideoLayerProps) {
  ...
  const setCurrentTime = usePlayerState(s => s.setCurrentTime);
  const setDuration = usePlayerState(s => s.setDuration);
  const setBuffered = usePlayerState(s => s.setBuffered);
  const setIsBuffering = usePlayerState(s => s.setIsBuffering);
  const setIsIntroFinished = usePlayerState(s => s.setIsIntroFinished);
