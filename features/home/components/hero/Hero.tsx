...
export default function Hero() {
  ...
  if (loading) {
    return (
      <div className="relative h-screen min-h-screen w-full bg-black flex items-center justify-center">
  ...
  return (
    <section className="relative h-screen min-h-screen w-full overflow-hidden bg-[#0a0a0a] group">
        ...
        >
          ...
          <div className="absolute inset-0 z-0">
            <Image
              ...
              fill
              priority
              className="object-cover object-top brightness-[0.65] transition-transform duration-[10000ms] ease-linear scale-100 group-hover:scale-110"
              sizes="100vw"
            />
            {/* Top Vignette Layer */}
            <div className="absolute inset-x-0 top-0 bg-gradient-to-b from-[#0a0a0a] via-[#0a0a0a]/60 to-transparent h-1/3" />
            {/* Bottom Vignette Layer */}
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/80 to-transparent h-1/2" />
          </div>

          {/* Main Content Layer */}
          <div className="relative z-10 h-full container mx-auto px-4 sm:px-6 md:px-[60px] flex flex-col justify-end pb-16 sm:pb-20 lg:pb-24">
            <motion.div
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-4 pt-2">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={() => router.push(`/${isTV ? 'series' : 'movie'}/${currentSlide.id}`)}
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-30 flex items-center gap-3">
        {slides.map((_, i) => (
          ...
          >
            <div className={`transition-all duration-500 rounded-full ${i === currentIndex
                ? 'w-[16px] h-[16px] bg-red-600 shadow-[0_0_15px_rgba(220,38,38,0.8)]'
                : 'w-[8px] h-[8px] bg-white/50 hover:bg-white/80 hover:scale-110'
              }`} />
          </button>
        ))}
      </div>
