...
import { Loader2, Camera, ChevronUp, ChevronDown, Grid, LayoutList, Trash2, PlayCircle, Video, Clock, CheckCircle2, XCircle, LogIn } from 'lucide-react';
...
export function ReelsPlayer({ initialReactions = [], movieId }: ReelsPlayerProps) {
  ...
  }, [isLoading, isFetchingMore, hasMore, movieId]);
  ...
  const fetchUserReactions = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      setUserReactions(Array.isArray(data) ? data : []);
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    const confirmed = window.confirm('Are you sure you want to delete this reaction?');
    if (!confirmed) return;
  ...
  useEffect(() => {
    ...
    fetchUserReactions();
  }, [fetchMore, fetchUserReactions, initialReactions.length]);
    ...
    <div className="relative w-full h-[100dvh] bg-[#050505] flex flex-col pt-6 pb-2">
      ...
      <div className="w-full max-w-5xl mx-auto flex justify-between items-end px-8 shrink-0 mb-6">
        ...
        <button
          onClick={() => {
            if (status !== 'authenticated') {
              toast.error('Please sign in to post a reaction.');
              return;
            }
        ...
        >
          ...
          {status === 'authenticated' ? 'Post Yours' : 'Sign In to Post'}
        </button>
      </div>
          ...
          <div className="w-full max-w-5xl h-full overflow-y-auto px-8 pb-12 custom-scrollbar">
            {status !== 'authenticated' ? (
              <div className="h-full flex flex-col items-center justify-center text-zinc-500 gap-4">
                ...
                <p>Sign in to view and manage your reactions.</p>
              </div>
