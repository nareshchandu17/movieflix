...
const ActressDetailPage: React.FC<ActressDetailPageProps> = ({ actressName }) => {
  const [actress, setActress] = useState<TMDBPerson | null>(null);
  ...
  useEffect(() => {
    const fetchActressDetails = async () => {
      try {
        setLoading(true);
        
        // Search for the actress
        const person = await tmdbAPI.searchPerson(actressName);
        if (!person) {
        ...
        // Fetch actress's movies using secure API route
        const response = await fetch(`/api/tmdb/person/${person.id}/credits`);
        
        if (!response.ok) {
          throw new Error("Failed to fetch actress movies");
        }
        
        const data = await response.json();
        const actressMovies = data.cast
        ...
        setMovies(actressMovies);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Failed to load actress details");
        console.error("Error fetching actress details:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-black via-gray-950 to-black flex items-center justify-center">
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-8 text-center max-w-md">
          <p className="text-red-400 text-lg">{error || "Actress not found"}</p>
          <button
            onClick={() => router.back()}
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-950 to-black">
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 py-6">
        ...
        >
          ...
          <span className="text-xl font-bold">Back to Actresses</span>
        </button>
      </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="flex-shrink-0">
              <div className="relative w-64 h-64 lg:w-80 lg:h-80 rounded-2xl overflow-hidden border-4 border-white/20 shadow-2xl shadow-pink-500/20">
                <Image
                  src={tmdbAPI.getProfileURL(actress.profile_path)}
                  alt={actress.name}
                  fill
            ...
            <div className="flex-1 space-y-6">
              <div>
                <h1 className="text-4xl lg:text-5xl font-bold text-white mb-2">{actress.name}</h1>
                <p className="text-xl text-gray-400 mb-4">{actress.known_for_department}</p>
              </div>
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-gray-400">Complete filmography of {actress.name}</p>
          </div>
          ...
          {movies.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {movies.map((movie) => (
                <Link
                  ...
                  className="block"
                >
                  <div className="relative aspect-[2/3] rounded-lg overflow-hidden border-2 border-white/10 transition-all duration-300 hover:scale-105 hover:border-pink-500/50 hover:shadow-2xl hover:shadow-pink-500/20">
                    <Image
            ...
            <div className="text-center py-12">
              <p className="text-gray-400">No movies found for this actress.</p>
            </div>
          )}
        </div>
      </div>
