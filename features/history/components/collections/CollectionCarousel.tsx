      ...
      <div className="relative group/section py-8">
        ...
        <div className="px-4 sm:px-6 md:px-12 lg:px-20">
            ...
            >
              ...
              <p className="text-gray-400 font-medium text-sm">Browse movies to add to your collection</p>
            </div>
          </div>
      </div>
    );
  ...
  return (
    <div className="relative group/section py-8">
      ...
      <div className="relative">
        <button
          onClick={() => scroll('left')}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); scroll('left'); } }}
          tabIndex={0}
          aria-label="Scroll previous items"
          className="absolute left-0 top-0 bottom-0 w-12 md:w-16 bg-black/60 backdrop-blur-md z-40 opacity-0 group-hover/section:opacity-100 transition-opacity duration-300 flex items-center justify-center text-white focus:outline-none focus:ring-2 focus:ring-red-500/50"
        >
        ...
        >
          {items.map((item, index) => (
            ...
            >
              ...
              >
                <Image
                  src={item.posterPath ? (item.posterPath.startsWith('http') ? item.posterPath : `https://image.tmdb.org/t/p/w500${item.posterPath}`) : 'https://images.unsplash.com/photo-1485846234645-a62644f84728?auto=format&fit=crop&q=80&w=400'}
      ...
      <button
        ...
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); scroll('right'); } }}
        tabIndex={0}
        aria-label="Scroll next items"
        className="absolute right-0 top-0 bottom-0 w-12 md:w-16 bg-black/60 backdrop-blur-md z-40 opacity-0 group-hover/section:opacity-100 transition-opacity duration-300 flex items-center justify-center text-white focus:outline-none focus:ring-2 focus:ring-red-500/50"
      >
