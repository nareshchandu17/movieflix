...
const HeroTrailer = ({ 
  ...
  useEffect(() => {
    const savedMutePreference = localStorage.getItem('heroTrailerMuted');
  ...
  useEffect(() => {
    localStorage.setItem('heroTrailerMuted', JSON.stringify(isMuted));
  }, [isMuted]);
  ...
  const shouldAutoPlay = useCallback(() => {
    return !isMobile && !networkSlow && !isLowPowerMode && isVisible;
  }, [isMobile, networkSlow, isLowPowerMode, isVisible]);

  // Handle video playback based on visibility and conditions
  useEffect(() => {
    ...
    if (shouldAutoPlay() && !hasUserInteracted) {
      ...
      // Pause video if conditions aren't met
      if (isPlaying) {
        video.pause();
        setIsPlaying(false);
      }
  ...
  }, [shouldAutoPlay, hasUserInteracted, isVisible]);
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && videoRef.current && isPlaying) {
        ...
        setIsPlaying(false);
      } else if (!document.hidden && shouldAutoPlay() && !hasUserInteracted) {
  ...
  }, [isPlaying, shouldAutoPlay, hasUserInteracted, isVisible]);
  ...
  const handleVideoError = () => {
    console.log('Video failed to load, using poster fallback');
    setVideoReady(false);
  };
