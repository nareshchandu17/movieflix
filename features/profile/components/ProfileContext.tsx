...
import type { Profile, CreateProfilePayload } from '@/features/profile/types/profiles';
  ...
  const fetchProfiles = useCallback(async () => {
    ...
    try {
      ...
      if (profilesData.success) {
        ...
        let activeProfileFound: Profile | null = null;
        ...
        if (!activeProfileFound) {
          ...
          if (lastUsedData.success && lastUsedData.lastUsedProfile) {
            ...
            // If found, set it as active profile if no PIN is required
            if (activeProfileFound) {
              if (!activeProfileFound.pinEnabled) {
                try {
                  await selectProfile(activeProfileFound);
                } catch (e) {
                  console.warn("Could not auto-select last used profile:", e);
                }
              }
            }
          }
