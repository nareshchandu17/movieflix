  ...
  useEffect(() => {
    const fetchAnime = async () => {
      try {
        ...
        const animeMovies = ((animeMoviesResponse.results || []) as TMDBMovie[]).map((movie) => ({
          ...movie,
          media_type: "movie" as const,
        }));

        const animeTVShows = ((animeTVResponse.results || []) as TMDBTVShow[]).map((show) => ({
          ...show,
          media_type: "tv" as const,
        }));

        setAnime([...animeTVShows, ...animeMovies].slice(0, 40));
      } catch (error) {
