...
const HeroTrailerAuto = ({ 
  ...
  useEffect(() => {
    if (!videoRef.current || isMobile || hasUserInteracted || autoPlayAttempted) return;
    ...
    const tryAutoPlay = async () => {
      ...
      try {
        ...
        await video.play();
        console.log('Auto-play successful (method 1)');
        setIsPlaying(true);
      } catch (error1) {
        console.log('Auto-play method 1 failed:', error1);
        
        try {
          // Method 2: Load then play
          await video.load();
          await video.play();
          console.log('Auto-play successful (method 2)');
          setIsPlaying(true);
        } catch (error2) {
          console.log('Auto-play method 2 failed:', error2);
          
          try {
            // Method 3: Set currentTime and play
            video.currentTime = 0.1;
            await video.play();
            console.log('Auto-play successful (method 3)');
            setIsPlaying(true);
          } catch (error3) {
            console.log('All auto-play methods failed:', error3);
          }
        }
      }
    };

    // Start immediately when video is ready
  ...
  }, [isMobile, hasUserInteracted, autoPlayAttempted]);
    ...
    >
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="absolute top-4 left-4 bg-black/50 text-white p-2 rounded text-xs z-30">
          ...
          <div>Auto-play Attempted: {autoPlayAttempted ? 'Yes' : 'No'}</div>
        </div>
      )}
    </div>
  );
...
export default HeroTrailerAuto;
