...
const RepliesList = ({ movieId, parentId, session }: { movieId: string, parentId: string, session: any }) => {
    const { comments, reactToComment, deleteComment } = useComments(movieId, parentId, 'oldest');
                ...
                <div key={reply.id} className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-gray-400 text-xs">{reply.createdAtRelative}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                onClick={() => session ? reactToComment(reply.id, 'like') : alert('Login required')}
                                className={`flex items-center gap-1 text-xs transition-colors ${reply.isLiked ? 'text-red-400' : 'text-gray-400 hover:text-red-300'}`}
                            >
                                <ThumbsUp className={`w-4 h-4 ${reply.isLiked ? 'fill-current' : ''}`} />
                                <span>{reply.likes}</span>
                            </button>
                            <button
                                onClick={() => session ? reactToComment(reply.id, 'dislike') : alert('Login required')}
                                className={`flex items-center gap-1 text-xs transition-colors ${reply.isDisliked ? 'text-red-400' : 'text-gray-400 hover:text-red-300'}`}
                            >
...
const EnhancedMovieInfo = ({ id }: EnhancedMovieInfoProps) => {
    ...
    const mappedSort = sortBy === 'recent' ? 'newest' : sortBy === 'liked' ? 'top' : 'newest';
    
    // Dynamic comments hook
    const { comments, addComment, deleteComment, reactToComment } = useComments(id.toString(), null, mappedSort);
    ...
    const handleLikeComment = async (commentId: string) => {
        if (!session) return toast.error("Please log in to react");
    ...
    const handleReportComment = (commentId: string) => {
        if (!session) return toast.error("Please log in to report");
    ...
    const handlePostReply = async (parentCommentId: string) => {
        if (!session) return toast.error("Please log in to reply");
        if (replyText[parentCommentId]?.trim()) {
            await addComment(replyText[parentCommentId], parentCommentId);
    ...
    useEffect(() => {
        const fetchMovieData = async () => {
            try {
                ...
                // Fetch real scenes from YouTube via our backend API
                try {
                    const scenesResponse = await fetch(`/api/scenes?q=${encodeURIComponent(movie.title + ' movie best clips scenes')}&maxResults=6`);
                    if (scenesResponse.ok) {
                        const scenesData = await scenesResponse.json();
                        if (scenesData.clips && scenesData.clips.length > 0) {
                            const ytScenes = scenesData.clips.map((clip: any) => ({
                                url: `https://www.youtube.com/watch?v=${clip.id}`,
                                title: clip.title,
                                duration: clip.duration || "HD",
                                description: `${clip.channel || "YouTube"} • ${clip.views || "1M+"} views`,
                                videoId: clip.id,
                                thumbnail: clip.thumbnail || `https://img.youtube.com/vi/${clip.id}/hqdefault.jpg`
                            }));
                            setScenes(ytScenes);
                        } else {
                            throw new Error("No clips found in YouTube API");
                        }
                    } else {
                        throw new Error("Failed to fetch scenes from API");
                    }
                } catch (sceneErr) {
                    console.warn("Falling back to mock scenes due to API error:", sceneErr);
                    // Fallback to mock scenes if YouTube API fails
            ...
            "action": [
                {
                    ...
                    videoId: "6WA2WG8dP8U",
                    thumbnail: "https://img.youtube.com/vi/6WA2WG8dP8U/hqdefault.jpg",
                    relevance: 0.9
                },
                {
                    ...
                    videoId: "wQiFp5kxh6I",
                    thumbnail: "https://img.youtube.com/vi/wQiFp5kxh6I/hqdefault.jpg",
                    relevance: 0.85
                },
                {
                    ...
                    videoId: "QKj3Dq0E8iM",
                    thumbnail: "https://img.youtube.com/vi/QKj3Dq0E8iM/hqdefault.jpg",
                    relevance: 0.8
                },
                {
                    ...
                    videoId: "Sa_Bd_4GqAo",
                    thumbnail: "https://img.youtube.com/vi/Sa_Bd_4GqAo/hqdefault.jpg",
                    relevance: 0.75
                },
                {
                    ...
                    videoId: "Y1W2M-8S6zU",
                    thumbnail: "https://img.youtube.com/vi/Y1W2M-8S6zU/hqdefault.jpg",
                    relevance: 0.7
                },
                {
                    ...
                    videoId: "8QnSpd_h5q4",
                    thumbnail: "https://img.youtube.com/vi/8QnSpd_h5q4/hqdefault.jpg",
                    relevance: 0.65
                }
            ],
            "thriller": [
                ...
                },
                {
                    ...
                    videoId: "7Wtq0mCq9V8",
                    thumbnail: "https://img.youtube.com/vi/7Wtq0mCq9V8/hqdefault.jpg",
                    relevance: 0.85
                },
                {
                    ...
                    videoId: "2Vqk4WkMJKk",
                    thumbnail: "https://img.youtube.com/vi/2Vqk4WkMJKk/hqdefault.jpg",
                    relevance: 0.8
                },
                {
                    ...
                    videoId: "9FzhyqQgJ2k",
                    thumbnail: "https://img.youtube.com/vi/9FzhyqQgJ2k/hqdefault.jpg",
                    relevance: 0.75
                }
            ],
            "comedy": [
                {
                    ...
                    videoId: "J--dVDm4d7E",
                    thumbnail: "https://img.youtube.com/vi/J--dVDm4d7E/hqdefault.jpg",
                    relevance: 0.9
                },
                {
                    ...
                    videoId: "H7c8jB3_2jQ",
                    thumbnail: "https://img.youtube.com/vi/H7c8jB3_2jQ/hqdefault.jpg",
                    relevance: 0.85
                },
                {
                    ...
                    videoId: "M3nK8oPeNpw",
                    thumbnail: "https://img.youtube.com/vi/M3nK8oPeNpw/hqdefault.jpg",
                    relevance: 0.8
                },
                {
                    ...
                    videoId: "4LjZ4Z1s8Wk",
                    thumbnail: "https://img.youtube.com/vi/4LjZ4Z1s8Wk/hqdefault.jpg",
                    relevance: 0.75
                }
            ],
            "drama": [
                {
                    ...
                    videoId: "8WgU2y9bK7M",
                    thumbnail: "https://img.youtube.com/vi/8WgU2y9bK7M/hqdefault.jpg",
                    relevance: 0.9
                },
                {
                    ...
                    videoId: "3HqQk7L4VvE",
                    thumbnail: "https://img.youtube.com/vi/3HqQk7L4VvE/hqdefault.jpg",
                    relevance: 0.85
                },
                {
                    ...
                    videoId: "5Kz8R5X2YmQ",
                    thumbnail: "https://img.youtube.com/vi/5Kz8R5X2YmQ/hqdefault.jpg",
                    relevance: 0.8
                },
                {
                    ...
                    videoId: "7F2d2K8q3pM",
                    thumbnail: "https://img.youtube.com/vi/7F2d2K8q3pM/hqdefault.jpg",
                    relevance: 0.75
                },
                {
                    ...
                    videoId: "9N3p7VrD8sQ",
                    thumbnail: "https://img.youtube.com/vi/9N3p7VrD8sQ/hqdefault.jpg",
                    relevance: 0.7
                }
            ],
            "romance": [
                {
                    ...
                    videoId: "2VpBj9bF3wU",
                    thumbnail: "https://img.youtube.com/vi/2VpBj9bF3wU/hqdefault.jpg",
                    relevance: 0.9
                },
                {
                    ...
                    videoId: "4DqK8rN2sXQ",
                    thumbnail: "https://img.youtube.com/vi/4DqK8rN2sXQ/hqdefault.jpg",
                    relevance: 0.85
                },
                {
                    ...
                    videoId: "6Fp9jR7sYpM",
                    thumbnail: "https://img.youtube.com/vi/6Fp9jR7sYpM/hqdefault.jpg",
                    relevance: 0.8
                },
                {
                    ...
                    videoId: "8Hq3K5mT9sQ",
                    thumbnail: "https://img.youtube.com/vi/8Hq3K5mT9sQ/hqdefault.jpg",
                    relevance: 0.75
                },
                {
                    ...
                    videoId: "9LmX4nN7sRm",
                    thumbnail: "https://img.youtube.com/vi/9LmX4nN7sRm/hqdefault.jpg",
                    relevance: 0.7
                }
            ],
            "horror": [
                {
                    ...
                    videoId: "3DqK8rN2sXQ",
                    thumbnail: "https://img.youtube.com/vi/3DqK8rN2sXQ/hqdefault.jpg",
                    relevance: 0.9
                },
                {
                    ...
                    videoId: "5Hq3K5mT9sQ",
                    thumbnail: "https://img.youtube.com/vi/5Hq3K5mT9sQ/hqdefault.jpg",
                    relevance: 0.85
                },
                {
                    ...
                    videoId: "7Fp9jR7sYpM",
                    thumbnail: "https://img.youtube.com/vi/7Fp9jR7sYpM/hqdefault.jpg",
                    relevance: 0.8
                },
                {
                    url: "https://www.youtube.com/watch?v=9LmX4nN7sRm",
                                        ...
                                        <div className="glass-card-sm-light p-4 flex items-center gap-6">
                                            ...
                                            <div className="flex gap-3">
                                                {movieInsights.movieMood.map((mood, index) => (
                                                    <span key={mood} className={`px-4 py-1.5 rounded-full text-[10px] font-bold border border-white/10 transition-all duration-300 ${index === 0 ? "bg-pink-500/20 text-pink-500 border-pink-500/30 shadow-[0_0_10px_rgba(255,46,99,0.2)]" : "bg-white/5 text-gray-400 hover:bg-white/10"
                                                        }`}>
                                                        {mood}
                                                    </span>
                                                ))}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div className="lg:col-span-8">
                            ...
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <div className="relative dropdown-container">
                                    ...
                                    >
                                        ...
                                        <ChevronRight
                                            className={`w-4 h-4 text-white/70 transition-transform duration-300 ${isDropdownOpen ? 'rotate-90' : ''
                                                }`}
                                        />
                                    </button>
                                            ...
                                            <div className="py-1">
                                                {[
                                                    ...
                                                    <button
                                                        ...
                                                        className={`w-full px-4 py-2.5 text-left hover:bg-white/10 transition-colors duration-200 ${sortBy === option.value ? 'bg-white/5 text-white' : 'text-white/80 hover:text-white'
                                                            }`}
                                                    >
                            ...
                            <div className="flex gap-4 mb-8">
                                ...
                                <div className="flex-1">
                                    <div className={`border-b pb-1 transition-all duration-300 ${newComment.trim()
                                        ...
                                        <textarea
                                            ...
                                            className={`w-full bg-transparent text-white resize-none focus:outline-none placeholder-gray-500 text-sm py-1 transition-all duration-300 ${newComment.trim()
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center justify-between mb-1">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-gray-400 text-xs">{comment.createdAtRelative}</span>
                                                    </div>
                                                    {comment.isOwnComment && !comment.isDeleted && (
                                                        <button
                                                ...
                                                {/* Comment Text */}
                                                <div className={`text-sm leading-relaxed mb-2 ${comment.isDeleted ? 'text-gray-500 italic' : 'text-gray-300'}`}>
                                                    {comment.text}
                                                </div>
                                                ...
                                                {!comment.isDeleted && (
                                                    <div className="flex items-center gap-4">
                                                        <button
                                                            ...
                                                            className={`flex items-center gap-1 text-xs transition-colors ${comment.isLiked ? 'text-blue-400' : 'text-gray-400 hover:text-gray-300'
                                                                }`}
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            className={`flex items-center gap-1 text-xs transition-colors ${comment.isDisliked ? 'text-blue-400' : 'text-gray-400 hover:text-gray-300'
                                                                }`}
                                                        >
                                                ...
                                                {replyingTo === comment.id && (
                                                    <div className="mt-4">
                                                        <div className="flex gap-4">
                                                            ...
                                                            {/* Reply Input Area */}
                                                            <div className="flex-1">
                                                                <div className={`border-b pb-1 transition-all duration-300 ${replyText[comment.id]?.trim()
                                                                        ? 'border-red-500 shadow-[0_0_8px_rgba(239,68,68,0.3)]'
                                                                        : 'border-gray-600'
                                                                    }`}>
                                                                    <textarea
                                                                        ...
                                                                        className={`w-full bg-transparent text-white resize-none focus:outline-none placeholder-gray-500 text-sm py-1 transition-all duration-300 ${replyText[comment.id]?.trim()
                                                                                ? 'text-white shadow-[0_0_12px_rgba(239,68,68,0.2)]'
                                                                                : ''
                                                                            }`}
                                                                        rows={1}
                                            ...
                                            <div className="relative w-full h-full bg-gradient-to-br from-gray-800 to-gray-900">
                                                <Image
                                                    ...
                                                    onError={(e) => {
                                                        ...
                                                        target.src = `https://img.youtube.com/vi/${scene.videoId}/hqdefault.jpg`;
                                                    }}
                                                    onLoad={(e) => {
