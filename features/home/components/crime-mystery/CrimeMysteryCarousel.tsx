    ...
    const fetchData = async () => {
      try {
        const responses = await Promise.all([
          ...
          api.getMedia("movie", { category: "popular", genre: "9648" }),
        ]);

        const all: (TMDBMovie & { media_type: "movie" })[] = [];

        responses.forEach((res) => {
          const list = (res.results || []) as TMDBMovie[];
          all.push(...list.map((m) => ({ ...m, media_type: "movie" as const })));
        });

        const unique = all.filter((m, i, self) => i === self.findIndex((x) => x.id === m.id)).slice(0, 40);
        setMovies(unique);
      } catch (err) {
        console.error(err);
      } finally {
