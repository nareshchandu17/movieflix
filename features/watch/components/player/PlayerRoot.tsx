...
import { useAutoHideControls } from '@/features/watch/hooks/useAutoHideControls';
import { useProgressPersistence } from '@/features/history/hooks/useProgressPersistence';
import { useChapterDetection } from '@/features/watch/hooks/useChapterDetection';
import { useReactionRecorder } from '@/features/social/hooks/useReactionRecorder';
import { useGestureControls } from '@/features/watch/hooks/useGestureControls';
...
import { ReactionOverlay } from '@/features/social/components/reactions/ReactionOverlay';
import { OfficialViewingScreen } from './OfficialViewingScreen';
...
export function PlayerRoot({ contentId, url, title, roomId, isWatchParty, contentData }: PlayerRootProps) {
  // Initialize hooks
  ...
  const isBuffering = usePlayerState(s => s.isBuffering);
  const controlsVisible = usePlayerState(s => s.controlsVisible);
  const activeReaction = usePlayerState(s => s.activeReaction);
  const reactionLayout = usePlayerState(s => s.reactionLayout);
  const activePanel = usePlayerState(s => s.activePanel);
  const isIntroFinished = usePlayerState(s => s.isIntroFinished);
  
  const showControls = usePlayerState(s => s.showControls);
  const hideControls = usePlayerState(s => s.hideControls);
  const seek = usePlayerState(s => s.seek);
  const togglePlay = usePlayerState(s => s.togglePlay);
  const setActiveReaction = usePlayerState(s => s.setActiveReaction);
  const setMovieReactions = usePlayerState(s => s.setMovieReactions);
  const togglePanel = usePlayerState(s => s.togglePanel);
  const setIsFullscreen = usePlayerState(s => s.setIsFullscreen);
  ...
  const handleInteraction = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (e.detail === 1) {
      ...
      if (isLeftThird) {
        const nextTime = usePlayerState.getState().currentTime - 10;
        seek(nextTime);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ReactionRecorder 
          ...
          onSuccess={(newReaction) => {
            setMovieReactions((prev: unknown[]) => [...prev, newReaction]);
          }}
        />
      ...
      <AnimatePresence>
        {isIntroFinished && contentData && (
          <OfficialViewingScreen 
            contentData={contentData} 
            providers={contentData.providers || {}} 
          />
        )}
      </AnimatePresence>
    </div>
