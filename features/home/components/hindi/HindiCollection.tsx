...
import { hindiApi, HindiMovie } from "@/features/home/services/hindiApi";
...
const HindiCollection = () => {
  const [hindiMovies, setHindiMovies] = useState<HindiMovie[]>([]);
  ...
  const fetchMovies = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const movies = await hindiApi.fetchHindiMovies(250);
      setHindiMovies(movies);
    } catch (err) {
    ...
    return (
      <div className="w-full">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-3xl font-bold text-white mb-2">Hindi Collection</h2>
            <p className="text-gray-400">Premium Hindi cinema experience</p>
          </div>
          <Link 
            href="/hindi"
  ...
  const carouselMovies = hindiMovies.slice(0, 20);
    ...
    <div className="w-full">
      <div className="relative group">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-3xl font-bold text-white mb-2">Hindi Collection</h2>
            <p className="text-gray-400">{hindiMovies.length}+ Premium Hindi movies</p>
          </div>
          <Link 
            href="/hindi"
            className="flex items-center gap-2 text-red-500 hover:text-red-400 transition-colors duration-300 group"
          ...
          <div className="relative group">
            ...
            >
              {carouselMovies.map((movie, index) => {
                // Convert HindiMovie to TMDBMovie format for MediaCard
                const mediaItem = {
                  id: movie.id,
                  title: movie.title,
