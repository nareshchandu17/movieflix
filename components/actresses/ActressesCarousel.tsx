...
const ActressesCarousel: React.FC = () => {
  ...
  useEffect(() => {
    const fetchActresses = async () => {
      try {
        setLoading(true);
        const actressesData = await tmdbAPI.getMultipleActors(TOP_20_ACTRESSES);
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-3xl font-bold text-white mb-2">Top 20 Great Actresses</h2>
          <p className="text-gray-400">Discover stunning performances from cinema's finest talents</p>
        </div>
      ...
      <div className="relative left-0 right-1/2 -mr-[50vw] w-[calc(100vw+2rem)]">
        <div className="relative group">
          ...
          <button
            onClick={() => scroll("left")}
            className="absolute left-6 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/80 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-pink-600/80 transition-all duration-300 opacity-100 z-20 border border-white/10 hover:border-pink-500/50"
          ...
          <button
            ...
            className="absolute right-6 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/80 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-pink-600/80 transition-all duration-300 opacity-100 z-20 border border-white/10 hover:border-pink-500/50"
          ...
          >
            {loading ? (
              ...
              actresses.map((actress) => (
                <div key={actress.id} className="snap-start flex-shrink-0" style={{ width: `${cardWidth}px` }}>
                  <ActressCard
                    name={actress.name}
...
export default ActressesCarousel;
